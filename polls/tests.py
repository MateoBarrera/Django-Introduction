...
class QuestionsModelTests(TestCase):
  ...
  def test_was_published_recently_with_past_questions(self):
    """was_published_recently returns False for questions whose pub_date is in the past (more than a day)
    """
    time = timezone.now() - datetime.timedelta(days=30)
    past_question = Question(question_text="Test Question?", pub_date=time)
    self.assertIs(past_question.was_published_recently(), False)
    
  def test_was_published_recently_with_recently_questions(self):
    """was_published_recently returns True for questions whose pub_date is recently (less than a day)
    """
    time = timezone.now()
    past_question = Question(question_text="Test Question?", pub_date=time)
...
def create_question(question_text, days):
  """Create a question with the given text, and published the given number of days to now.

  Args:
      question_text (_type_): text for the question
      days (_type_): number of days (negative for questions published in the past, positive for questions that have yet to be published)
  """
...
class QuestionIndexViewTests(TestCase):
  def test_no_questions(self):
    ...
    self.assertContains(response, "No hay polls disponibles.")
...
class QuestionDetailViewTest(TestCase):
  ...
  def test_past_question(self):
    """The detail view of a question with a pub_data in the past displays the question's text.
    """
    past_question = create_question("past question", days=-30)
